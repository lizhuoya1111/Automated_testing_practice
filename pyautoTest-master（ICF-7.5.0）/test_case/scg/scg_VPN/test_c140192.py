...
def test_c140192(browser):
	...
	try:
		...
		for au in ["md5", "sha1", "sha256", "sha384", "sha512", "sm3"]:
			...
			edit_ipsec_remote_gateway_gm(browser, name='test1', advanced='yes', ah='yes',
			                             ah_auth_alg=au, return1="yes", save='yes')
			out_en_au = get_ipesc_remote_gateway_en_au_gm(browser, name='test1', switch="阶段2", per2_switch="AH")
