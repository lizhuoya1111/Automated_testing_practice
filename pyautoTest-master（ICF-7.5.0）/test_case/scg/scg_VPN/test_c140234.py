...
def test_c140234(browser):
	...
	try:
		...
		add_ipsec_remote_access(browser, name='tt2', desc='miaoshu', status='enable', out_interface=interface_name_1,
		...
		del_ipsec_remote_access(browser, name='tt2')
		loginfo2 = get_log_info(browser, 管理日志)

		try:
			assert "成功删除远程接入隧道" in loginfo1
			assert "成功删除远程接入隧道" in loginfo2
