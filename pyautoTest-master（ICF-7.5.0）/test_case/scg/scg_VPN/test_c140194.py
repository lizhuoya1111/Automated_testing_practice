	...
	try:
		...
		if oem_name == "SCG":
			...
			add_ipsec_remote_gateway_gm(browser, name='test1', desc='miaoshu', status='enable',
										...
										advanced='yes', ipsec_sa="time", ipsec_sa_lifetime='864001', save='yes')
			time.sleep(0.5)
			# 获取失败提示信息
			...
			add_ipsec_remote_gateway_gm(browser, name='test1', desc='miaoshu', status='enable',
										...
										remote='remote_ip', remote_ip='25.12.1.0/255.255.255.0',
										ipsec_super_net_switch="no",
										advanced='yes', ipsec_sa="time", ipsec_sa_lifetime=random_lifetime, save='yes')
			...
			add_ipsec_remote_gateway_wxw(browser, name='test1', desc='miaoshu', status='enable',
										 ...
										 remote_addr_obj='any',
										 advanced='yes',
										 ipsec_sa='time', ipsec_sa_lifetime='119',
										 save='yes', cancel='')
			time.sleep(0.5)
			# 获取失败提示信息
			result1 = browser.find_element_by_xpath('//*[@id="box"]/div[3]/ul/li[2]').text
			...
			add_ipsec_remote_gateway_wxw(browser, name='test1', desc='miaoshu', status='enable',
										 ...
										 remote_addr_obj='any',
										 advanced='yes',
										 ipsec_sa='time', ipsec_sa_lifetime='864001',
										 save='yes', cancel='')
			time.sleep(0.5)
			# 获取失败提示信息
			result2 = browser.find_element_by_xpath('//*[@id="box"]/div[3]/ul/li[2]').text
			...
			add_ipsec_remote_gateway_wxw(browser, name='test1', desc='miaoshu', status='enable',
										 ...
										 remote_addr_obj='any',
										 advanced='yes',
										 ipsec_sa='time', ipsec_sa_lifetime=random_lifetime,
										 save='yes', cancel='')
