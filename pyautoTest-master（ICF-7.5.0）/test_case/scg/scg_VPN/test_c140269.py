...
def test_c140269(browser):
	...
	try:
		...
		if oem_name == "ICF" or oem_name != "SCG":
			...
			add_ipsec_remote_gateway_gm(browser, name='ipsec_jia_1', desc='miaoshu', status='enable',
										out_interface=interface_name_2,
			...
			add_ipsec_remote_gateway_gm(browser, name='ipsec_jia_2', desc='miaoshu', status='enable',
										out_interface=interface_name_2,
										remote_gateway='static', gateway='10.2.2.82', auth_method='证书',
										localid="CN=127.0.0.1", remoteid="CN=127.0.0.2", remote_cert_id_any_switch="no",
										local='local_ip', local_ip='30.1.1.0/255.255.255.0',
		...
		webinfo = is_ipsec_remote_gateway_exist_wxw(browser, name='ipsec_jia_1')
		webinfo1 = is_ipsec_remote_gateway_exist_wxw(browser, name='ipsec_jia_2')

		del_ipsec_remote_gateway_wxw(browser, name='ipsec_jia_1')
		del_ipsec_remote_gateway_wxw(browser, name='ipsec_jia_2')
		try:
			assert webinfo is True
			assert webinfo1 is True
