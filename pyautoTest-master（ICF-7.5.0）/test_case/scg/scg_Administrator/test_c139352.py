...
# 2.添加admin user，login type是https，ssh，引用该profile
# 3.使用该用户登录https，查看和通过web写配置
# 4.使用该用户登录ssh
# 通过https登录只能查看配置，通过ssh登录设备也只能查看配置---如何验证？？
#


def test_c139352(browser):
    try:
        # 登录函数
        ...
        add_admin_lzy(browser, admin_name="lzy", auth_database="local", temp="lzy", https="yes", telent="no",
                      ssh="yes", console="no", password="admin@139", confirm_password="admin@139", status="enable",
