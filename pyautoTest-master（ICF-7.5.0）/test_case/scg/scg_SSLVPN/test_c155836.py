...
test_id = "155836"
# SSLVPN-安全站点-增加
# 输入非法站点名称，例如空格 $ < > \ ' '' | ; , ? & # ￥或者超过32个字符，其余配置均正确，点击保存。
# 不能保存且有错误提示
def test_c155836(browser):
    try:
        login_web(browser, url=dev1)
        # 删除所有安全站点（避免冲突）
        del_all_sslvpn_safe_site_lzy(browser)
        # 增加安全站点，站点名称为$
        add_sslvpn_safe_site_lzy(browser, sitename=" $<\|;,?&#￥'", siteip=dev1, siteport='9090', enable='yes',
        ...
        try:
            assert "名称输入错误" in alert1 and num1 == 0
