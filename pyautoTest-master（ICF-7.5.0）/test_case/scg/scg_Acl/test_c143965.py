...
def test_c143965(browser):
	try:
		...
		add_acl_rule_complete_wxw(browser, aclgroup_name='acl_group', source_zone_interface=interface_name_4 + ".2",
								  source_custom='yes', fromip='20.1.1.2', fromnetmask='255.255.255.255', source_address_object='no',
								  dest_address_object='yes', d_address_object='A:any',
								  dest_zone_interface="Z:any",
								  service='P:HTTP', schdule='-- 无 --',
								  accept='yes', drop='no',
								  auth='-- 无 --', icf='no', log='no', save='yes', cancel='no')
		# time.sleep(300)
		ssh_pc2 = Shell_SSH()
		ssh_pc2.connect(hostip="10.1.1.202", name="root", passwd="root", po=22)
		ssh_pc2.execute('ifconfig ' + server_pc_2_eth1 + ' 20.1.1.2 netmask 255.255.255.0')
		...
		ssh_pc2.execute('telnet 21.1.1.2 80')
		time.sleep(1)
		ssh_pc2.execute('quit')
		time.sleep(1)
		pc2topc3_http_info = ssh_pc2.output()
		# print(pc2topc3_http_info)

		ssh_pc2.execute('ifconfig ' + server_pc_2_eth1 + ' 20.1.1.3 netmask 255.255.255.0')

		ssh_pc2.execute('telnet 21.1.1.2 80')
		time.sleep(2)
		ssh_pc2.execute('quit')
		time.sleep(1)
		pc2topc3_http_info_2nd = ssh_pc2.output()
		# print(pc2topc3_http_info_2nd)
		...
		try:
			assert "Server: Apache" in pc2topc3_http_info and "Server: Apache" not in "pc2topc3_http_info_2nd"
