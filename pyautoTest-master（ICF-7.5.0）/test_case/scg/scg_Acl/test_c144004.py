...
# 1.添加一个address对象a---1.可以添加
# 2.添加一条acl，源为该address对象a，目的为all，服务为all，动作为accept，从源地址a穿过DUT进行访问，如ping 外网地址---2.可以添加，能ping通外网
# 3.修改地址对象a，观察源地址a处的pc ping外网的情况---3.不能ping通
# PS： 本测试用例为ping另一台设备 不是ping外网


def test_c144004(browser):
    try:
