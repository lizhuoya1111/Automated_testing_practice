...
def test_c141664(browser):
	...
	try:
		...
		while int(n) <= int(int(getnum1)+1):
			get_interface_num = browser.find_element_by_xpath('//*[@id="table"]/tbody/tr[ ' + str(n) + ' ]/td[5]').text
			n = n + 1
			get_infor.append(get_interface_num)
		# print(get_infor)
		try:
			assert "Auto / -- / --" or "Auto / 100M / Full" or "Auto / 1000M / Full" in get_infor
