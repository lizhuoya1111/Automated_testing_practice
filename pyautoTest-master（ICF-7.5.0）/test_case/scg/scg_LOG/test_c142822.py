...
test_id = 142822
# 1.在security日志中，设置查询条件，包括：关键字，时间日期，每页记载的日志数，goto，advanced下的日志级别，需要记日志的模块，将查询条件命名为“安全日志查询”
# 2.删除这个查询条件
# 2.可以删除，有日志记录，shell显示正确

# 设备初始状态没有安全日志，需更改日志过滤 再通过IPMac绑定产生安全日志再做查询操作 注意还原时：IPMac（绑定列表及修改）、日志过滤、删除查询条件
def test_c142822(browser):
    try:
        ...
        login_web(browser, url=dev1)
        # 配置查询条件并保存为“安全日志查询”
        advanced_query_save_security_or_traffic_log_lzy(browser, log_type=安全日志, advanced="yes", ambiguous_search="",
                                                        ...
                                                        debug="yes/no", module="change", modul_name="neighmacipmacbind",
                                                        save_as="yes", save_as_name="安全日志查询")
        # 点击该条件查询安全日志
        advanced_query_save_modify_admin_log_lzy(browser, log_type=安全日志, refresh="yes", refresh_name="安全日志查询",
                                                 delete="no")
        # 获取模块 判断为neighmacipmacbind
        sleep(0.5)
        ...
        try:
            assert "neighmacipmacbind" in info1 and 'Delete' in log1
