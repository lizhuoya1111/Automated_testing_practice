...
test_id = 142829
# 开启Log Switch后，检查符合do not match rules的log是否会显示在Traffic类型的log中
def test_c142829(browser):
    try:
        ...
        # 先用81ping83（直连ping 无日志）
        sign_out_jyl(browser)
        login_web(browser, url=dev1)
        sleep(1)
        diag_ping(browser, ipadd="13.1.1.3", interface=interface_name_3)
