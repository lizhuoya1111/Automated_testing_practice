...
def test_c143586(browser):
	...
	try:
		...
		physics_interface_pppoe_set(browser, physical_interface=interface_name_4, pppoe_uesr="test", pppoe_passwd="test", dns_renew="yes")
		loginfo = get_log_info(browser, log_type=管理日志, num=3)
		try:

			assert "启动PPPoE成功，参数: 用户名 [test]，密码 [******]，空闲时间 [0]，DNS是否更新 [1]." in loginfo
