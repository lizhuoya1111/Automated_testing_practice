...
def test_c139589(browser):
	...
	try:
		...
		del_dynamic_arp_by_index(browser, index=5)
		loginfo = get_log_info(browser, log_type=管理日志)

		shell_cmd = Shell_SSH()
		shell_cmd.connect(hostip=dev2)
		for x in [81, 34, 83, 84, 85, 86]:
			shell_cmd.ping_cmd(ipadd="10.2.2." + str(x))
			time.sleep(2)
		# qq = shell_cmd.output()
		# print(qq)
		shell_cmd.close()
		del_dynamic_arp(browser)
		loginfo1 = get_log_info(browser, log_type=管理日志)

		try:
			assert "成功删除动态条目，参数: [ip:10.2.2." in loginfo and "成功删除动态" in loginfo1
