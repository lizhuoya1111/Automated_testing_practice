...
test_id = "141118"
# 添加一条策略路由，enable和disable该路由条目（操作包括添加时enable、disable；页面list里enable、disable按钮；编辑enable、disable）
# enable时，该路由条目生效；不勾选时，路由条目失效，通信无法正常


def test_c141118(browser):
    ...
    try:
        ...
        add_policy_route_single_wxw(browser, in_device='全部', src_ip='12.1.1.0', src_mask='24',
                                    ...
                                    out_device=interface_name_3, gateway='13.1.1.3', enable='no',
        ...
        # 修改策略路由为启用
        login_web(browser, url=dev1)
        edit_policy_route_single_wxw(browser, destination='34.1.1.0/255.255.255.0',
                                     in_device='全部', src_ip='12.1.1.0', src_mask='24',
                                    dst_ip='34.1.1.0', dst_mask='24', service='yes', serv='any',
                                    service_grp='no', serv_grp='H323',
        ...
        try:
            assert "Unr" in result1 and "ms" in result2
