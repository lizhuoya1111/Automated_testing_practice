...
test_id = "141141"
# 验证策略路由克隆功能，克隆一条策略路由分修改参数、不修改参数两种情况
# 添加生效


def test_c141141(browser):
    ...
    try:
        ...
        # 克隆 修改参数 启用
        login_web(browser, url=dev1)
        clone_policy_route_by_Index_number_complete_lzy(browser, Index_number='1',
                                                        clonemult='yes', m_in_device='全部', m_src_ip='12.1.1.0',
                                                        m_src_mask='24',
                                                        m_dst_ip='34.1.1.0', m_dst_mask='24', m_service='yes', m_serv='any',
                                                        m_service_grp='no', m_serv_grp='H323',
                                                        m_gw_group='1(GROUP_1)', m_grp_mem=["主用", "备份1"], m_enable='yes',
                                                        m_desc='maioshu', m_save='yes', m_cancel='no',
                                                        clonesingle='no', s_in_device='全部', s_src_ip='',
                                                        s_src_mask='',
        ...
        try:
            assert "Un" in result1 and "Un" in result2 and 'ms' in result3
