...
# list里selecet all删除所有ISP文件
# 所有ISP条目被删除，数据包不再匹配ISP路由


def test_c141173(browser):
    ...
    try:
        ...
        # 添加ISP
        for x in range(1,4):
            add_multi_isp_save_wxw(browser, name='lzy'+str(x))

        # 导入IP
        import_ip_config_file_wxw(browser, name='lzy1', save='yes', cancel='no', file='34.1.1.0.txt')

        # 添加路由 单网关
        add_isp_route_lzy(browser, name='lzy1', single_gateway='yes', device=interface_name_3, gateway='13.1.1.3',
        ...
        result1 = diag_ping(browser, ipadd="34.1.1.4", packersize="100", count="5", ping_wait_time="2",
                            interface=interface_name_3, timesleep=2.5)
        # print(result1)
