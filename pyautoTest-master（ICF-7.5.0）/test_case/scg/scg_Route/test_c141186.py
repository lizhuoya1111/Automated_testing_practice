...
test_id = "141186"
# Route maitance group gateway 功能 13
# gateway group里的link server 为gateway ip
# 可以正常判断server 状态，并且影响被引用的路由


def test_141186(browser):
	...
	try:
		...
		for n in range(4, 6):
			...
			add_multi_gateway_group_wxw(browser, name='mgg_186_'+str(n), group="1(GROUP_1)", modify='no', alias='',
									    device=interface_name_3, gateway='34.1.1.'+str(n), ping_server='34.1.1.4', ping='yes',
									    arp='no', time_switch='7', ub="100000", db="100000")

		status1 = get_multi_gateway_status_gateway_wxw(browser, name='mgg_186_4')
		# print(status1)
		...
		edit_multi_gateway_group_wxw(browser, name='mgg_186_4', group="1(GROUP_1)", modify='yes/no', alias='',
		...
		status2 = get_multi_gateway_status_gateway_wxw(browser, name='mgg_186_4')
		# print(status2)
