...
def test_c139807(browser):
	...
	try:
		...
		add_snat(browser, name="snat_jia_1", desc="", src_inter_zone='Z:IPSEC_ANY', des_inter_zone=interface_name_3,
		...
		try:
			...
			assert '从区域端口 [IPSEC_ANY]' in add_acl_loginfo
			assert '配置SNAT对象成功' in add_sant_loginfo
			assert '从域/接口 [IPSEC_ANY]' in add_sant_loginfo
