...
test_id = 142212
# 同时修改输入次数，锁住时间和interval的配置，看是否能修改
# 全局设置
def test_c142212(browser):
    try:
        login_web(browser, url=dev5)
        # 全局设置
        add_globel_setting_lzy(browser, frozen="yes", frozen_time="30", retry="1", interval_time="30",
                               phone_interval="1200", idle_time="60", sms_center="", HTTP_port="80",
        ...
        try:
            assert "aaa global setting" in log1 and "http_port=80 is_frozen=1" in log2 and "frozen_time=30(min) retry=1 interval=30" in log1
