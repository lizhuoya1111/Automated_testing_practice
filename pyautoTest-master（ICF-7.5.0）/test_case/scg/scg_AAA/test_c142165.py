...
test_id = 142165
# 设置Radius的认证方式是PAP，看是否能下发，在shell中查看是否与ui配置一致，查看log记录是否正确

def test_c142165(browser):
    try:
        ...
        add_RADIUS_aut_server_lzy(browser, server_name="lzy", description="abcdef", server_address="10.0.0.1", backup_host_1="10.0.0.2",
                                  backup_host_2="10.0.0.3", port="2005", retry="0", timeout="2", password="adf123", PAP="yes",
                                  CHAP="yes/no", charging="yes/no", save="yes", cancel="yes/no")
        ...
        try:
            assert "PAP" in info1 and "成功添加 aaa server" in log1
