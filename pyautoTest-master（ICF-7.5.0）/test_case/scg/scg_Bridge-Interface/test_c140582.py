...
test_id = 140582
def test_c140582(browser):
	try:
		login_web(browser, url=dev1)
		bridge_add_jyl(browser, bridge_name="br_abc")
		bri_add_jyl(browser)
		# 清除默认输入
		browser.find_element_by_xpath('//*[@id="name"]').clear()
		# 输入网桥名称
		browser.find_element_by_xpath('//*[@id="name"]').send_keys("br_ABC")
		# 点击保存
		...
		a.execute("en")
		a.execute("conf t")
		a.execute("no interface bridge br_abc")
		a.execute("exit")

		try:
			assert "已经存在" in webinfo1
