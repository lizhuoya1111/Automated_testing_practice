...
test_id = "144213"
# ACL 5
# 添加一条ACL,目的地址被设定为addr1,action为accept,尝试删除该obj
# 匹配源地址的包可以过,该obj不能被删除


def test_c144213(browser):
	...
	try:
		...
		# 添加一个addr
		add_obj_address_wxw(browser, name='obj_add_213', desc='zhe是yi个描述1', subnetip='34.1.1.0', subnetmask='24')

		# 添加防火墙组
		add_acl_group_complete(browser, name='acl_group_213', enable='yes', sour='Z:any', dest='Z:any', desc='miaoshu',
							   save='yes', cancel='no')
		# 添加acl规则
		add_acl_rule_complete_wxw(browser, aclgroup_name='acl_group_213', source_zone_interface='Z:any',
								  source_custom='no', fromip='', fromnetmask='',
								  source_address_object='yes', s_address_object='A:any',
								  mac='',
								  dest_custom='no', toip='', tonetmask='',
								  dest_address_object='yes', d_address_object='A:obj_add_213',
		...
		# 禁用该条防火墙
		enable_acl_group_wxw(browser, aclgroup_name='acl_group_213', enable='no')
		...
		del_obj_address_wxw(browser, name='obj_add_213')
		time.sleep(2)
		...
		del_acl_group_wxw(browser, name='acl_group_213')
