...
def test_c144227(browser):
	try:
		login_web(browser, url=dev1)
		# 自定义的object service（server的FTP端口），并添加一个组
		...
		add_obj_serv_grp_wxw(browser, name='obj_serv_ftp', desc='', serv_obj='C:obj_serv_ftp')
		...
		add_acl_rule_complete_wxw(browser, aclgroup_name='acl_group', source_zone_interface="Z:any",
		                          ...
		                          service='G:obj_serv_ftp', schdule='-- 无 --',
		                          accept='yes', drop='no',
		...
		modify_obj_serv_grp_by_name(browser, name='obj_serv_ftp', reduce='yes', serv_obj2='C:obj_serv_ftp')
		...
		ssh_pc2.close()
		del_ser_grp_info = del_obj_serv_grp_wxw(browser, name='obj_serv_ftp', alert_info="yes")
		# del_ser_obj_info = del_obj_service_wxw(browser, name='obj_serv_ftp', alert_info='yes')

		try:
			assert "230 Login successful." in pc2topc3_ftp_info and \
					"230 Login successful." not in pc2topc3_ftp_info_2 and \
					"对象正在使用" in del_ser_grp_info
