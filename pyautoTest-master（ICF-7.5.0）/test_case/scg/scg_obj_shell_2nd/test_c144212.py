...
test_id = "144212"
# ACL 4
# 添加一条ACL,源地址被设定为addr grp1,action为drop,尝试删除该obj
# 匹配源地址的包不可以过,该obj不能被删除


def test_c144212(browser):
	...
	try:
		...
		# 添加一个addr组
		add_obj_address_wxw(browser, name='obj_add_212', desc='zhe是yi个描述1', subnetip='13.1.1.0', subnetmask='24')
		add_obj_group_use_addr_obj_wxw(browser, name='obj_grp_212', desc='zhe是yi个描述1', addr_obj='A:obj_add_212')

		# 添加防火墙组
		add_acl_group_complete(browser, name='acl_group_212', enable='yes', sour='Z:any', dest='Z:any', desc='miaoshu',
							   save='yes', cancel='no')
		# 添加acl规则
		add_acl_rule_complete_wxw(browser, aclgroup_name='acl_group_212', source_zone_interface='Z:any',
								  source_custom='no', fromip='', fromnetmask='',
								  source_address_object='yes', s_address_object='G:obj_grp_212',
								  mac='',
		...
		del_obj_grp_wxw(browser, name='obj_grp_212')
		time.sleep(2)
		...
		del_acl_group_wxw(browser, name='acl_group_212')

		del_obj_grp_wxw(browser, name='obj_grp_212')

		del_obj_address_wxw(browser, name='obj_add_212')
