...
test_id = "144214"
# ACL 6
# 添加一条ACL,目的地址被设定为addr1,action为drop,尝试删除该obj
# 匹配源地址的包不可以过,该obj不能被删除


def test_c144214(browser):
	...
	try:
		...
		# 添加一个addr
		add_obj_address_wxw(browser, name='obj_add_214', desc='zhe是yi个描述1', subnetip='34.1.1.0', subnetmask='24')

		# 添加防火墙组
		add_acl_group_complete(browser, name='acl_group_214', enable='yes', sour='Z:any', dest='Z:any', desc='miaoshu',
							   save='yes', cancel='no')
		# 添加acl规则
		add_acl_rule_complete_wxw(browser, aclgroup_name='acl_group_214', source_zone_interface='Z:any',
								  source_custom='no', fromip='', fromnetmask='',
								  source_address_object='yes', s_address_object='A:any',
								  mac='',
								  dest_custom='no', toip='', tonetmask='',
								  dest_address_object='yes', d_address_object='A:obj_add_214',
		...
		del_obj_address_wxw(browser, name='obj_add_214')
		time.sleep(2)
		...
		del_acl_group_wxw(browser, name='acl_group_214')
