...
def test_main(browser):
	try:
		...
		time.sleep(1)
		bridge_add_jyl(browser, bridge_name="br_1", bridge_describe="", snat="", allow_ping="yes")
		...
		birdge_edit_interface_delete_jyl(browser, interface="ge0/5", bridge_interface="br_1")
		...
		ssh = SSH('10.1.1.212', 'root', 'root', 22)
		ssh.connect()
		result4 = ssh.execute('ping 21.1.1.4 -c 3')
		print(result4)
		# 关闭连接
		ssh.close()
		...
		ssh = SSH('10.1.1.212', 'root', 'root', 22)
		ssh.connect()
		result5 = ssh.execute('ping 21.1.1.3 -c 3')
		print(result5)
		# 关闭连接
		ssh.close()
		...
		try:
			...
			assert "100% packet loss" in result4
			assert "100% packet loss" in result5
