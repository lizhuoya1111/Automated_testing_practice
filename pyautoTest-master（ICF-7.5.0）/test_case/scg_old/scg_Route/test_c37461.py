...
test_id = 37461
# 静态多网关路由配置GW输入异常检测 6
# 网关group里有多网关，只有一个选active，其余都选unselect
...
def test_route_wxw(browser):
	...
	try:
		...
		for n in range(1, 5):
			...
			add_multi_gateway_group_wxw(browser, name='mgg_461_'+str(n), group="1(GROUP_1)", modify='no', alias='',
		...
		add_static_route_multi_gateway_before_save_wxw(browser, ip='192.168.188.3', mask='32', gateway_grp='1(GROUP_1)',
													   num=4, grp_mem=["主用", "不选择", "不选择", "不选择"], enable="no")
