...
def configuer(browser):
	...
	time.sleep(2)
	add_admin_jyl(browser, admin_name="bob", auth_database="local", temp="aaa", https="yes", telent="no",ssh="yes", console="yes", status="enable", interface="any", online_num="3", ip1="0.0.0.0/0")
	sys_set_jyl(browser, frozen_time="600", retry="3")
	for x in range(1, 4):
		login_web(browser, username="bob", password="admin@139" + str(x))
		browser.find_element_by_xpath('//*[@id="link_but"]').click()
		time.sleep(8)
...
if __name__ == '__main__':
	pytest.main(["-v", "-s", "test_c21583.py"])
