...
def test_admin_user_add(browser):
	...
	try:
		assert "配置地址对象成功，添加内部对象 [obj_add_1]" in loginfo
...
def configuer(browser):
	...
	add_admin(browser, admin_name="zxcvbnmlkjhgfdsaqwertyuiopzxcvb", temp="aaa")
	login_web(browser, username="zxcvbnmlkjhgfdsaqwertyuiopzxcvb")
	add_obj_address_wxw(browser)
	time.sleep(5)
	# 切换到默认frame
	browser.switch_to.default_content()
...
if __name__ == '__main__':
	pytest.main(["-v", "-s", "test_c21633.py"])