...
def test_admin_user_add(browser):
	...
	# 输入密码
	browser.find_element_by_xpath('//*[@id="newpwd"]').send_keys("0123456789012345678901234567890123456789012345678901234567zcvbnm@")
	# 确认密码
	browser.find_element_by_xpath('//*[@id="cfpwd"]').send_keys("0123456789012345678901234567890123456789012345678901234567zcvbnm@")
	browser.find_element_by_xpath('//*[@id="permitip1"]').send_keys("0.0.0.0/0")
	time.sleep(3)
	# 点击保存
	...
	try:
		assert "请输入密码，密码长度为8-64" in web_info
...
if __name__ == '__main__':
	pytest.main(["-v", "-s", "test_c21646.py"])