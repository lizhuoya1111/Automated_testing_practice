...
def test_main(browser):
	try:
		...
		if num == "0":
			dhcp_server_add_jyl(browser, interface="ge0/3", dhcp_type="dhcp_rely", dhcp_rely_server1="143.1.1.4",)
			dhcp_server_edit_or_delete_jyl(browser, fuction="edit", dhcp_type="rely", dhcp_rely_server1="13.1.1.6")
			time.sleep(2)
		else:
			i = 0
			while i < int(num):
				...
				if enable == True:
					...
					time.sleep(2)
					i += 1
			dhcp_server_add_jyl(browser, interface="ge0/3", dhcp_type="dhcp_rely", dhcp_rely_server1="13.1.1.4",)
			dhcp_server_edit_or_delete_jyl(browser, fuction="edit", dhcp_type="rely", dhcp_rely_server1="13.1.1.5")
			time.sleep(2)
